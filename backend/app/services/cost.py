...
TTS_PRICE_PER_MCHAR = {"azure": 16.0, "elevenlabs": 180.0, "mock": 0.0}  # à confirmer
...
def estimate(total_seconds: float, n_targets: int, model: str, tts_provider: str, device: str = "cpu") -> dict:
    n_seg = max(1, total_seconds / SEGMENT_SECONDS) * n_targets
    chars = total_seconds * CHARS_PER_SPOKEN_SECOND * n_targets * RETRY_FACTOR
    translate = claude_cost(model, int(n_seg * TOKENS_PER_SEGMENT_IN * RETRY_FACTOR),
